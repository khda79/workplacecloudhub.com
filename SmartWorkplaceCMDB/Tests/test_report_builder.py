...
class ReportTests(unittest.TestCase):
    ...
    def test_six_pages_have_working_bindings_and_keep_source_immutable(self):
        ...
        self.assertTrue((output / "SmartWorkplaceCMDB.Report").is_dir())
        self.assertTrue((output / "SmartWorkplaceCMDB.SemanticModel").is_dir())
        self.assertFalse((output / "CMDB-REPORTS.Report").exists())
        self.assertFalse((output / "CMDB-REPORTS.SemanticModel").exists())
        pbip = json.loads((output / "SmartWorkplaceCMDB.pbip").read_text(encoding="utf-8"))
        self.assertEqual(pbip["artifacts"][0]["report"]["path"], "SmartWorkplaceCMDB.Report")
        report_definition = json.loads((output / "SmartWorkplaceCMDB.Report/definition.pbir").read_text(encoding="utf-8"))
        self.assertEqual(report_definition["datasetReference"]["byPath"]["path"], "../SmartWorkplaceCMDB.SemanticModel")
        ...
        model = json.loads((output / "SmartWorkplaceCMDB.SemanticModel/model.bim").read_text(encoding="utf-8"))
