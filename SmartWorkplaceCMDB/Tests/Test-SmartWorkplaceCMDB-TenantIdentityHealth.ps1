...
$domainCollector=Join-Path $projectRoot 'Collectors\Entra\SmartWorkplaceCMDB-EntraVerifiedDomains-Collect.ps1'
$normalizer=Join-Path $projectRoot 'Collectors\Entra\SmartWorkplaceCMDB-EntraVerifiedDomains-Normalize.ps1'
...
$tempRoot=Join-Path ([IO.Path]::GetTempPath()) ('SmartWorkplaceCMDB-TenantHealth-{0}' -f [guid]::NewGuid().ToString('N'))
...
try {
    ...
    Invoke-Test 'Validate collector without output writes' {
        & $domainCollector @identity -InputJsonPath (Join-Path $fixtures 'EntraVerifiedDomains.sample.json') -ValidateOnly | Out-Null
        Assert-True (-not (Test-Path (Join-Path $tempRoot 'DATA-LAST'))) 'ValidateOnly created DATA-LAST.'
    }
    Invoke-Test 'Collect only verified domains' {
        & $domainCollector @identity -InputJsonPath (Join-Path $fixtures 'EntraVerifiedDomains.sample.json') | Out-Null
        $rows=@(Import-Csv (Join-Path $tempRoot 'DATA-LAST\Raw\Entra\Entra_VerifiedDomains.csv'))
        Assert-True ($rows.Count -eq 2) 'Expected exactly two verified domains.'
        Assert-True (@($rows | Where-Object IsDefault -eq 'true').Count -eq 1) 'Expected one default domain.'
        Assert-True ($rows.DomainId -notcontains 'pending.example') 'Unverified domain was published.'
    }
    Invoke-Test 'Publish domain dimension and aggregate hybrid coverage' {
        & $adCollector @identity -InputJsonPath (Join-Path $fixtures 'ActiveDirectory.sample.json') | Out-Null
        & $userCollector @identity -InputJsonPath (Join-Path $fixtures 'EntraUsers.sample.json') | Out-Null
        & $deviceCollector @identity -InputJsonPath (Join-Path $fixtures 'EntraDevices.sample.json') | Out-Null
        & $normalizer @identity | Out-Null
        $domains=@(Import-Csv (Join-Path $tempRoot 'DATA-LAST\PowerBI\DimVerifiedDomain.csv'))
        $coverage=@(Import-Csv (Join-Path $tempRoot 'DATA-LAST\PowerBI\FactHybridIdentityCoverage.csv'))
        Assert-True ($domains.Count -eq 2) 'Expected two domain dimension rows.'
        Assert-True ($coverage.Count -eq 2) 'Expected user and device coverage rows.'
        $users=@($coverage | Where-Object EntityType -eq 'User')[0]
        Assert-True ($users.OnPremisesCount -eq '3' -and $users.CloudCount -eq '2') 'User population counts are wrong.'
    ...
    Invoke-Test 'Reject duplicate verified domain identities' {
        ...
        Assert-True ($null -ne $caught -and $caught.Exception.Message -match 'duplicate') 'Duplicate domain identity was not rejected.'
    }
}
finally {
    if (Test-Path -LiteralPath $tempRoot) { Remove-Item -LiteralPath $tempRoot -Recurse -Force -ErrorAction SilentlyContinue }
}
Write-Information "SmartWorkplaceCMDB tenant identity health tests completed. Version=$ScriptVersion; Passed=$passed; Failed=$failed" -InformationAction Continue
