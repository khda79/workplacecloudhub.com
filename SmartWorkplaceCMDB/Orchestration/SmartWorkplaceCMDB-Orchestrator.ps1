    ...
    try {
        ...
        & $Step.ScriptPath @Parameters *>&1 | ForEach-Object {
            ...
            elseif ($record -is [System.Management.Automation.WarningRecord]) {
                $script:SmartWorkplaceCMDBCurrentWarningCount++
                'WARN'
            }
...
$script:SmartWorkplaceCMDBCurrentWarningCount = 0
...
if ($FinalizeOnly -and ($Collect -or $ValidateOnly -or
        -not [string]::IsNullOrWhiteSpace($FixtureRootPath))) {
    throw '-FinalizeOnly cannot be combined with -Collect, -ValidateOnly, or -FixtureRootPath.'
}
if ($FinalizeOnly -and ($Pipeline -ne 'Full' -or $MaxItems -gt 0 -or
        $ValidateExistingOutputs)) {
    throw '-FinalizeOnly requires -Pipeline Full and cannot use -MaxItems or -ValidateExistingOutputs.'
}
...
$sharePointEnabled = [bool](Get-SmartWorkplaceCMDBOrchestratorSetting `
        ...
        -Name 'Enabled' `
        -DefaultValue $false)
$sharePointEligible = $mode -in @('Collect', 'Finalize') -and
    $MaxItems -eq 0 -and
...
$summaryEligible = $mode -in @('Collect', 'Finalize') -and
...
$catalog['IntuneAnalyticsCollect'] = [pscustomobject]@{
    Name = 'Intune update and Endpoint Analytics report collection'
    Kind = 'Collect'
    ScriptPath = Join-Path $projectRoot 'Collectors\Intune\SmartWorkplaceCMDB-IntuneAnalytics-Collect.ps1'
    FixtureName = 'IntuneAnalytics.sample.json'
}
$catalog['IntuneAnalyticsNormalize'] = [pscustomobject]@{
    Name = 'Intune update and Endpoint Analytics normalization'
    Kind = 'Normalize'
    ScriptPath = Join-Path $projectRoot 'Collectors\Intune\SmartWorkplaceCMDB-IntuneAnalytics-Normalize.ps1'
    FixtureName = ''
}
...
$catalog['ADIntuneCoverageNormalize'] = [pscustomobject]@{
    Name = 'Exact AD workstation to Intune coverage'
    Kind = 'Normalize'
    ScriptPath = Join-Path $projectRoot 'Collectors\Intune\SmartWorkplaceCMDB-ADIntuneCoverage-Normalize.ps1'
    FixtureName = ''
}
$catalog['M365CollaborationCollect'] = [pscustomobject]@{
    Name = 'Microsoft 365 collaboration collection'
    Kind = 'Collect'
    ScriptPath = Join-Path $projectRoot 'Collectors\M365\SmartWorkplaceCMDB-M365Collaboration-Collect.ps1'
    FixtureName = 'M365Collaboration.sample.json'
}
$catalog['M365CollaborationNormalize'] = [pscustomobject]@{
    Name = 'Microsoft 365 collaboration normalization'
    Kind = 'Normalize'
    ScriptPath = Join-Path $projectRoot 'Collectors\M365\SmartWorkplaceCMDB-M365Collaboration-Normalize.ps1'
    FixtureName = ''
}
$catalog['M365UserActivityCollect'] = [pscustomobject]@{
    Name = 'Microsoft 365 user activity collection'
    Kind = 'Collect'
    ScriptPath = Join-Path $projectRoot 'Collectors\M365\SmartWorkplaceCMDB-M365UserActivity-Collect.ps1'
    FixtureName = 'M365UserActivity.sample.json'
}
$catalog['M365UserActivityNormalize'] = [pscustomobject]@{
    Name = 'Microsoft 365 user activity normalization'
    Kind = 'Normalize'
    ScriptPath = Join-Path $projectRoot 'Collectors\M365\SmartWorkplaceCMDB-M365UserActivity-Normalize.ps1'
    FixtureName = ''
}
...
switch ($Pipeline) {
    'Full' {
        Add-SmartWorkplaceCMDBOrchestratorStep $selectedSteps $catalog @(
            ...
            'IntuneDevicesCollect', 'IntuneDevicesNormalize', 'ADIntuneCoverageNormalize',
            'IntuneHardwareCollect',
            'IntuneOperationalCollect', 'IntuneOperationalNormalize',
            'IntuneAnalyticsCollect', 'IntuneAnalyticsNormalize',
            'UserDeviceRelationships',
            'M365SkusCollect', 'M365SkusNormalize',
            'M365LicensesCollect', 'M365LicensesNormalize',
            'M365UserActivityCollect', 'M365UserActivityNormalize',
            'M365CollaborationCollect', 'M365CollaborationNormalize',
    ...
    'CuratedOnly' {
        Add-SmartWorkplaceCMDBOrchestratorStep $selectedSteps $catalog @(
            ...
            'ADIntuneCoverageNormalize',
            'IntuneOperationalNormalize',
            'IntuneAnalyticsNormalize',
            'M365UserActivityNormalize',
            'M365CollaborationNormalize',
            'Relationships', 'DataQuality', 'Dimensions', 'Build', 'Report'
        )
    }
}
...
if ($summaryEligible -and $mode -eq 'Collect') {
    try {
...
$preflight = Test-SmartWorkplaceCMDBPreflight `
    ...
    -ThrowOnFailure:($mode -in @('Collect', 'Finalize'))
$script:SmartWorkplaceCMDBCurrentWarningCount += $preflight.WarningCount
...
$script:SmartWorkplaceCMDBCurrentRunGuard = Enter-SmartWorkplaceCMDBRunGuard `
    ...
    -NoWrite:($mode -notin @('Collect', 'Finalize'))
...
$sharePointFailedFiles = @($sharePointRecords | Where-Object Status -eq 'Failed')
foreach ($failedUpload in $sharePointFailedFiles) {
    $failedUploadMessage = (
        "SmartWorkplaceCMDB SharePoint file publication failed. LocalFile='{0}'; RelativePath='{1}'; Error='{2}'." -f
        ([string]$failedUpload.LocalFilePath),
        ([string]$failedUpload.RelativePath),
        ([string]$failedUpload.Error)
    )
    if ($loggingEnabled) {
        Write-SmartWorkplaceCMDBTextLog -Path $orchestratorLogPath `
            -Message $failedUploadMessage `
            -Level WARN
    }
    Write-SmartWorkplaceCMDBConsole -Level WARN -Message $failedUploadMessage
}
if ($sharePointEligible) {
    Write-SmartWorkplaceCMDBConsole -Message (
...
elseif ($script:SmartWorkplaceCMDBCurrentWarningCount -gt 0 -or
    $sharePointFailureCount -gt 0 -or
...
if ($summaryEligible) {
    try {
        Write-SmartWorkplaceCMDBConsole -Message (
            "SmartWorkplaceCMDB full-collection summary started. Mode={0}." -f
            $mode)
        if ($loggingEnabled) {
            Write-SmartWorkplaceCMDBTextLog -Path $orchestratorLogPath -Message (
                "Full-collection summary started. Mode={0}." -f $mode)
        }
        $summaryInvocationParameters = @{}
        if ($mode -eq 'Finalize') {
            $summaryInvocationParameters['ReuseLatestSnapshot'] = $true
        }
        ...
        Write-SmartWorkplaceCMDBConsole -Message (
            "SmartWorkplaceCMDB full-collection summary email sent. SnapshotReused={0}; HTML='{1}'." -f
            [bool]$summaryResult.SnapshotReused,
            $summaryResult.HtmlPath
        )
        if ($loggingEnabled) {
            Write-SmartWorkplaceCMDBTextLog -Path $orchestratorLogPath -Message (
                "Full-collection summary email sent. SnapshotReused={0}; HTML='{1}'." -f
                [bool]$summaryResult.SnapshotReused,
                $summaryResult.HtmlPath)
        }
    }
...
# that the email can report its outcome. Publish the generated summary files
# and the log delta once more after the notification. This keeps SharePoint's
# DATA-LAST, DATA-ALL, and LOG-ALL copies complete without republishing the
# entire data roots.
if ($sharePointEligible -and $sharePointPublishedSnapshot.Count -gt 0) {
    try {
        $finalSharePointSnapshot = Get-SmartWorkplaceCMDBSharePointSnapshot `
            -RootPath @(
                $paths.DataAllRootPath,
                $paths.LatestOutputRootPath,
                $paths.LogRootPath
            )
        $finalSharePointFiles = @($finalSharePointSnapshot.Keys | Where-Object {
                -not $sharePointPublishedSnapshot.ContainsKey($_) -or
                $sharePointPublishedSnapshot[$_] -ne
                    $finalSharePointSnapshot[$_]
            })
        if ($summaryResult) {
            $finalSharePointFiles += @(
                [string]$summaryResult.HistoryPath,
                [string]$summaryResult.LatestPath,
                [string]$summaryResult.HtmlPath
            )
        }
        $finalSharePointFiles = @($finalSharePointFiles |
            Where-Object {
                -not [string]::IsNullOrWhiteSpace([string]$_) -and
                (Test-Path -LiteralPath $_ -PathType Leaf)
            } | Sort-Object -Unique)
        if ($finalSharePointFiles.Count -gt 0) {
            $finalSharePointRecords = @(
                Publish-SmartWorkplaceCMDBSharePointFile `
                    -LocalFilePath $finalSharePointFiles `
                    @sharePointPublishParameters
            )
            ...
            $finalSharePointFailureCount = @($finalSharePointRecords |
                Where-Object Status -eq 'Failed').Count
            Write-SmartWorkplaceCMDBConsole -Message (
                'SmartWorkplaceCMDB SharePoint summary and log synchronization completed. Uploaded={0}; Failed={1}.' -f
                @($finalSharePointRecords |
                    Where-Object Status -eq 'Uploaded').Count,
        ...
        $sharePointError = if ([string]::IsNullOrWhiteSpace($sharePointError)) {
            "Summary and log synchronization: $finalSharePointError"
        }
        else {
            "$sharePointError | Summary and log synchronization: $finalSharePointError"
        }
        if ($loggingEnabled) {
            Write-SmartWorkplaceCMDBTextLog -Path $orchestratorLogPath `
                -Message (
                    "SharePoint summary and log synchronization failed: $finalSharePointError") `
                -Level WARN
        }
        Write-SmartWorkplaceCMDBConsole -Level WARN -Message (
            "SmartWorkplaceCMDB SharePoint summary and log synchronization failed: $finalSharePointError")
    }
...
$completionWarningCount = $script:SmartWorkplaceCMDBCurrentWarningCount +
    $sharePointFailureCount +
...
# The completion banner is appended after the preceding synchronization. Send
# the exact orchestrator artifacts one final time so the SharePoint LOG-ALL
# copy ends with the same terminal status as the local log.
if ($sharePointEligible -and $null -ne $sharePointPublishParameters) {
    try {
        $terminalFiles = @(@($orchestratorLogPath, $logPath) | Where-Object {
                -not [string]::IsNullOrWhiteSpace([string]$_) -and
                (Test-Path -LiteralPath $_ -PathType Leaf)
            } | Sort-Object -Unique)
        if ($terminalFiles.Count -gt 0) {
            $terminalRecords = @(Publish-SmartWorkplaceCMDBSharePointFile `
                    -LocalFilePath $terminalFiles `
                    @sharePointPublishParameters)
            $sharePointRecords = @($sharePointRecords) + @($terminalRecords)
            $sharePointFailureCount = @($sharePointRecords |
                Where-Object Status -eq 'Failed').Count
            $sharePointUploadCount = @($sharePointRecords |
                Where-Object Status -eq 'Uploaded').Count
            if (@($terminalRecords |
                    Where-Object Status -eq 'Failed').Count -gt 0) {
                $runStatus = 'CompletedWithWarnings'
            }
        }
    }
    catch {
        $terminalSharePointError = $_.Exception.Message
        $sharePointError = if ([string]::IsNullOrWhiteSpace($sharePointError)) {
            "Terminal log synchronization: $terminalSharePointError"
        }
        else {
            "$sharePointError | Terminal log synchronization: $terminalSharePointError"
        }
        $runStatus = 'CompletedWithWarnings'
        Write-SmartWorkplaceCMDBConsole -Level WARN -Message (
            "SmartWorkplaceCMDB SharePoint terminal log synchronization failed: $terminalSharePointError")
    }
}
...
[pscustomobject]@{
    ...
    SummarySnapshotReused = if ($summaryResult) { [bool]$summaryResult.SnapshotReused } else { $false }
    SummaryEmailError = $summaryError
    FinalizationOnly = $mode -eq 'Finalize'
}
}
catch {
    Exit-SmartWorkplaceCMDBRunGuard `
    ...
    if (-not $script:SmartWorkplaceCMDBCompletionWritten) {
        ...
        Write-SmartWorkplaceCMDBCompletionBanner `
            ...
            -WarningCount $script:SmartWorkplaceCMDBCurrentWarningCount `
            -ErrorCount 1
        $script:SmartWorkplaceCMDBCompletionWritten = $true
    }
    throw
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCB/LTvpiHTd4t2v
# U7BiboQLhDO4YWB6IA7NbUOViIw4OaCCF/swggS9MIIDJaADAgECAhAebu87xzjh
...
# hvcNAQkEMSIEIKM3jlbyDZTaGFaukXidOOzm5fJFnBlwvsnM0P8viN9qMA0GCSqG
# SIb3DQEBAQUABIIBgFXyaVtL1l05eMFdJNK4tTmE03Cp6m8dg0DfqIHCGjsO275u
# +Es6hiGvHHmbgZ6D3f1lSeVV8YVupwRE9wu3LIpM/VYAOnolz16ipnBnMWhPSkhP
# k5Mw8H9X8nOOp8erVCQnCF/TnZ0oETKiIWlgBKfwvx36X1AhNYkJ+vXoeziSoglP
# w9RRX1mih7NRmPm7+EQOnv7hTOpd2uM/IjYQ3B6tvoPJGLaOPJuJqPx2Jt37TCSY
# 3exOXhc/DaItTwqW3XEsMEBMChAj9xcjxAsw+6ttFmLVR3wtEiini5pWQ3y6LxR7
# GqEk3FnV6JNJNLJBpLSUhXOw5W/EkDQvpk5Iph1Usasw1nSGw6TOMV9XR2Ge8rIp
# nuX9KEbv5JJGd1nl7P5h9ZfGFG/fZbeho+bpsmtz2Zjfip+f8WdfOvCF4lS3UzsE
# FvsGwPKz+lRskJyAmgARkRMxp3370Byb6gXZvgu/ldFPPsgVUzJSXZYSrSZ/AgMS
# chjiPpTyC/1VUjC7WaGCAyYwggMiBgkqhkiG9w0BCQYxggMTMIIDDwIBATB9MGkx
# CzAJBgNVBAYTAlVTMRcwFQYDVQQKEw5EaWdpQ2VydCwgSW5jLjFBMD8GA1UEAxM4
# RGlnaUNlcnQgVHJ1c3RlZCBHNCBUaW1lU3RhbXBpbmcgUlNBNDA5NiBTSEEyNTYg
# MjAyNSBDQTECEAhP3DNPfkVO28MPj/mSGDUwDQYJYIZIAWUDBAIBBQCgaTAYBgkq
# hkiG9w0BCQMxCwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yNjA5MTYwODI1
# NDNaMC8GCSqGSIb3DQEJBDEiBCCVdiZebhSb6545076MJj+MpQ7uN/yRyG7y7wd+
# 3Qgm3DANBgkqhkiG9w0BAQEFAASCAgBlvKnp4krfH9asKKzaI+ZEeyv4oBUZV+JO
# vjXcfHwSiXY0Z/QtudqWQSppur+m9XGL4Wc2YH1H3VpqDYj1hjyoPN+LNMNtnHFy
# mv8bJlzrQkhIEwcW6qKhiCvsTLppVv09OuWXywUBMsENz8elHzRQkJPm02iyXmxC
# g3yhnb7N0qk8Y8IdWCevWmlYhQGX/SSiqHW5ocpSmDiZVT9QAv+2q9h9pn4pzFhs
# +Zg/8LlScTHA42V1CbjvM5k7Urr/blaSp23tUCSPoU6SZbI739Sek1cnX5tV+qwB
# fD2Q1JZF+4nGlIaNe4Svzgbqzhuftukr0IBvZumaYfwIsC8ug0QOd3MVXhyV6jbj
# YzVJgfAv658ixHRt5d8jV0U/ypeqrTZacD6yKl9W4/2WXYDrDEykj5I1ZNJ8nAOs
# aIr4NSAp6SsfVb7H+/zHaMhnG8RMNKxj6YeKvP9fetF6hqR0K6OA6N+gZEN/qg9J
# JUe+RdXNTDjrNuhDMvu7rsrAmORSynvRrffL+amTbBhG+4i+z+fyTynBA6OTMGMR
# Q9JZG6x+DhBd3JrhkLKHKqlwh6P0JFvG5VwaYHfWDHIitAHvjsV3/beM2q5zz+6C
# nVkZB+9oqS/8HbkvDDlWhm1kUmGiC9rm/+AkQair+IY51xVUSU0kx68Sag1xTwEM
# UvijuQUKEg==
