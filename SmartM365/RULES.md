...
- Every inventory/report script must send a Teams `Infos` notification when it completes without error. The notification must include script name, tenant/organization when known, computer name, timestamp, output/CSV path when relevant, and a short completion summary.
