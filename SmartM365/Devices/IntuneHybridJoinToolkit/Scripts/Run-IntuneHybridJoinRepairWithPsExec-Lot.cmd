...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%SCRIPT%" ^
  ...
  %ALLOW_DSREG_LEAVE_ARG% ^
  %ALLOW_STALE_INTUNE_CLEANUP_ARG% ^
  %ALLOW_REBOOT_NO_USER_ARG% ^
  %ALLOW_REBOOT_AFTER_LEAVE_ARG% ^
  %IGNORE_RUN_GUARD_ARG% ^
  %RUN_ONCE_ARG% ^
  %SKIP_VM_ARG% ^
  %SKIP_PRE_RUN_ARCHIVE_ARG% ^
  %CONTINUE_DNS_PREFLIGHT_ARG% ^
  %PSEXEC_ARG% ^
