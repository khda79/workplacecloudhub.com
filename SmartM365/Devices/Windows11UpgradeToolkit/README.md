...
- Automatic LOT refreshes are limited to the current `Computers.txt` and write inventory CSV files inside the LOT folder.
...
LOT-*\CentralLogs\SetupMigrationPluginFailure\<Computer>\Latest
LOT-*\CentralLogs\SetupMediaManifestFailure\<Computer>\Latest
