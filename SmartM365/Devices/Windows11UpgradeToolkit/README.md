...
2. Optionally enter one or more short-name prefixes in **Computer prefix(es)**, for example
   `FR-` or `FR-;BE-`. Leave the field empty to keep all computer names. While the LOT name
...
- Intune export uses delegated interactive Microsoft Graph authentication. `W11UT_INTUNE_TENANT_ID` is an optional tenant-id constraint; no tenant profile, app id, or certificate thumbprint is required.
- `W11UT_SKIP_INTUNE_INVENTORY_REFRESH=1` is the default and uses an existing run/root CSV without starting Graph authentication during a LOT. Set it explicitly to `0` only when an operator is present and an interactive scoped refresh is intended.
