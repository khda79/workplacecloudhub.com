...
function NewTableEmailBody {
    param(
        [string]$Title,            # Email subject/title
        [hashtable]$SummaryData,   # Key-value pairs for the summary table
        [string]$Message           # Optional message displayed first
    )
...
function NewTableFilesEmailBody {
    param(
        [string]$Title,            # Email subject/title
        [hashtable]$SummaryData,   # Key-value pairs for summary table
        [array]$Files,             # Array of detailed file objects
        [string]$Message           # Optional message displayed first
    )

    $now = Get-Date -Format "yyyy-MM-dd HH:mm:ss"

    # Enrich each file object with RowCount if it's a CSV, and log the process
    foreach ($file in $Files) {
        if ($file.FileName -match '\.csv$') {
            try {
                $rowCount = (Get-Content $file.FullName | Measure-Object -Line).Lines - 1 # -1 for header
    ...
    # File details table rows (Rows after Size)
