...
function Get-SemrExchangeOnlineSessionInfo {
    ...
    try {
        ...
        $matching = @(if ($TenantId) { $connections | Where-Object { [string](Get-SemrPropertyValue -InputObject $_ -Names @('TenantID','TenantId') -Default '') -eq $TenantId } })
...
function Invoke-SemrAssessment {
    ...
    foreach ($row in $summaryRows) {
        ...
        [void]$summary.Add([pscustomobject][ordered]@{
            ...
            DataCoverage = Get-SemrDataCoverage -AssessmentStatus $assessmentStatus -MailboxFindings @($mailFindings) -GlobalFindings @($globalFindings) -MailboxBlockingFindings @($mailboxBlocking)
