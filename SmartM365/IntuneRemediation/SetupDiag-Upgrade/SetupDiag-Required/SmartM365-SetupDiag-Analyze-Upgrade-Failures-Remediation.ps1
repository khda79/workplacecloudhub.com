...
$targetDirectory = Join-Path -Path $env:ProgramFiles -ChildPath "SetupDiag"
...
function Download-SetupDiag {
    ...
    try {
        Write-Log "Attempting to download SetupDiag.exe from Microsoft."
        ...
        if (-not (Test-Path -LiteralPath $temporaryDownloadPath -PathType Leaf)) {
            Write-Log "SetupDiag download failed because the temporary file was not created."
            return $false
        }
        ...
        if ($downloadedFile.Length -lt 100KB) {
            Write-Log "SetupDiag download failed because the downloaded file is unexpectedly small. SizeBytes=$($downloadedFile.Length)"
            Remove-Item -LiteralPath $temporaryDownloadPath -Force -ErrorAction SilentlyContinue
            return $false
        }

        Move-Item -LiteralPath $temporaryDownloadPath -Destination $DestinationPath -Force
        Write-Log "SetupDiag.exe downloaded successfully. SizeBytes=$($downloadedFile.Length)"
        return $true
    }
    catch {
        Write-Log "SetupDiag download failed: $($_.Exception.Message)"
        return $false
    }
}
...
try {
    ...
    Write-Log "===== SetupDiag remediation started ====="

    if (-not (Test-Path -Path $targetDirectory)) {
        New-Item -ItemType Directory -Path $targetDirectory -Force | Out-Null
        Write-Log "Created SetupDiag target directory: $targetDirectory"
    }

    if (-not (Test-Path -LiteralPath $targetExecutable -PathType Leaf)) {
        Write-Log "SetupDiag.exe is missing from target directory. Probing local Windows Setup folders first."
        ...
        foreach ($candidate in $setupDiagCandidates) {
            if (Test-Path -LiteralPath $candidate -PathType Leaf) {
                ...
                Write-Log "Copied SetupDiag.exe from local candidate: $candidate"
        ...
        if (-not $setupDiagAvailable) {
            $setupDiagAvailable = Download-SetupDiag -DestinationPath $targetExecutable
        }

        if (-not $setupDiagAvailable -or -not (Test-Path -LiteralPath $targetExecutable -PathType Leaf)) {
            Write-Log "ERROR: SetupDiag.exe is not available locally and could not be downloaded."
            exit 1
        }
    }
    else {
        Write-Log "SetupDiag.exe is present: $targetExecutable"
    }

    if (-not (Test-Path -Path $OutputRoot)) {
        New-Item -ItemType Directory -Path $OutputRoot -Force | Out-Null
        Write-Log "Created SetupDiag output directory: $OutputRoot"
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($selectedLogRoot)) {
        Write-Log "Setup logs found. Running SetupDiag in offline mode. LogsPath=$selectedLogRoot"
        $setupDiagArguments += "/LogsPath:$selectedLogRoot"
        $executionMode = "Offline"
    }
    else {
        Write-Log "No setup logs found. Running SetupDiag in online mode."
    ...
    Write-Log "SetupDiag completed with exit code: $($setupDiagProcess.ExitCode)"
    ...
    if (Test-Path -LiteralPath $setupDiagResultPath -PathType Leaf) {
        try {
            ...
            Write-Log "SetupDiag XML result parsed successfully."
        }
        catch {
            Write-Log "WARNING: SetupDiag XML parsing failed: $($_.Exception.Message)"
        }
    }
    else {
        Write-Log "WARNING: SetupDiagResults.xml was not generated."
    }
    ...
    Write-Log "JSON summary written: $jsonPath"
    Write-Log "Registry summary written: $enterpriseRegistryPath"

    if (-not $summary.ResultFound) {
        Write-Log "SetupDiag completed but no usable XML result was found."
        exit 1
    }

    Write-Log "===== SetupDiag remediation completed successfully ====="
    exit 0
}
catch {
