...
| `DeviceManagementManagedDevices.ReadWrite.All` | Required by Microsoft's create `deviceManagementExportJob` documentation. Endpoint Analytics uses it only to POST a temporary report export job and never sends a device, configuration, policy, assignment, or remediation write. | `SmartInventory/M365Inventory/IntuneInventory/EndpointAnalytics/SmartM365-EndpointAnalytics-Inventory.ps1`; `SmartInventory/M365Inventory/IntuneInventory/WindowsUpdate/AutopatchAlerts/SmartM365-Intune-WindowsAutopatch-Alerts-Inventory.ps1`. |
