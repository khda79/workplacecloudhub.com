...
- `SmartInventory/M365Inventory/PowerBI/SmartM365-PowerBIFabricActivity-Inventory.ps1` uses the Power BI resource rather than Microsoft Graph. Its service principal must not be assigned admin-consent-required Power BI permissions. App-only authorization is granted through the Fabric tenant setting **Service principals can access read-only admin APIs**, scoped to a security group containing the service principal.
- `Intune/Remediation/GUI/SmartM365-IntuneRemediation-GUI.ps1` is intentionally outside the app-only model: it uses only interactive delegated authentication, requests `DeviceManagementScripts.ReadWrite.All` to create, update, list, duplicate, and delete Intune remediations, requests `DeviceManagementConfiguration.Read.All` to export execution reports, and requests `Group.Read.All` to enrich assignment exports with Entra group names. The current repository does not contain a separate remediation CLI.
...
The startup preflight for each script must log its required PowerShell modules and minimum runtime permissions, then verify them with import checks, Graph endpoint probes, Exchange RBAC probes, or AD/on-prem cmdlet probes before any business CSV is produced. If a required module cannot be installed/imported, or a permission probe fails, the script must stop with a failed status rather than exporting incomplete data.
...
| `User.Read.All` | Reads users and their properties for M365 exports and Exchange Online enrichment. | `SmartInventory/M365Inventory/Users/SmartM365-ActiveUsers-Inventory.ps1`; `SmartInventory/M365Inventory/Licensing/SmartM365-Licences-Inventory.ps1`; `SmartInventory/ExchangeInventory/Mailboxes/SmartM365-EXO-Mailboxes-Inventory.ps1`; `SmartInventory/ExchangeInventory/BackupProtection/SmartM365-M365-BackupProtectedMailboxes-Inventory.ps1`; `SmartInventory/ExchangeInventory/BackupProtection/SmartM365-M365-BackupPolicyScope-Inventory.ps1`. |
| `AuditLog.Read.All` | Reads sign-in activity and authentication-method registration details used for MFA, SSPR, and passwordless readiness evidence. | `SmartInventory/M365Inventory/Users/SmartM365-ActiveUsers-Inventory.ps1`; `SmartInventory/M365Inventory/Security/AuthenticationMethods/SmartM365-AuthenticationMethodsRegistration-Inventory.ps1`. |
| `BackupRestore-Configuration.Read.All` | Reads Microsoft 365 Backup protection units for mailbox, SharePoint site, and OneDrive protection inventory. | `SmartInventory/ExchangeInventory/BackupProtection/SmartM365-M365-BackupProtectedMailboxes-Inventory.ps1`; `SmartInventory/ExchangeInventory/BackupProtection/SmartM365-M365-BackupProtectedSitesAndDrives-Inventory.ps1`. |
| `Policy.Read.All` | Reads Conditional Access policy configuration and targeting. This provides configuration-scope evidence, not proof that a policy was applied to a particular sign-in. | `SmartInventory/M365Inventory/Security/ConditionalAccess/SmartM365-ConditionalAccess-Inventory.ps1`. |
| `SecurityEvents.Read.All` | Reads current and historical Microsoft Secure Score records and their control scores. | `SmartInventory/M365Inventory/Security/SecureScore/SmartM365-SecureScore-Inventory.ps1`. |
| `Reports.Read.All` | Reads Microsoft 365 usage reports, including active user, mailbox usage, OneDrive usage, SharePoint site usage, SharePoint user activity, Office activations, Teams user activity, Teams device usage, Teams team activity, email activity, and licensed Microsoft 365 Copilot user usage reports for FinOps and adoption analysis. | `SmartInventory/M365Inventory/Usage/SmartM365-M365UserActivity-Inventory.ps1`; `SmartInventory/M365Inventory/Usage/SmartM365-CopilotUsage-Inventory.ps1`; `SmartInventory/M365Inventory/SharePoint/SmartM365-SPO-Inventory.ps1`; `SmartInventory/M365Inventory/Teams/SmartM365-Teams-Inventory.ps1`. |
| `CallRecords.Read.All` | Reads Teams Phone PSTN Calling Plan, Operator Connect, and Direct Routing call-detail rows through the Graph v1.0 `getPstnCalls` and `getDirectRoutingCalls` functions. These APIs support application authentication only. | `SmartInventory/M365Inventory/Teams/SmartM365-TeamsPhonePstnUsage-Inventory.ps1`. |
...
| `Group.Read.All` | Reads Microsoft 365 groups, owners, members, and team-backed groups for Teams, RBAC, and assignment enrichment. | `SmartInventory/M365Inventory/Teams/SmartM365-Teams-Inventory.ps1`; `SmartInventory/M365Inventory/IntuneInventory/RBAC/SmartM365-Intune-RBAC-GroupMembers.ps1`; `SmartInventory/M365Inventory/IntuneInventory/SmartM365-Intune-ExportRemediationScripts.ps1` when assignment names are enriched. |
| `GroupMember.Read.All` | Reads Entra ID group members, useful for RBAC exports and group-based comparisons. | `SmartInventory/M365Inventory/IntuneInventory/RBAC/SmartM365-Intune-RBAC-GroupMembers.ps1`; `SmartInventory/ExchangeInventory/BackupProtection/SmartM365-M365-BackupProtectedMailboxes-Inventory.ps1`; `SmartInventory/ExchangeInventory/BackupProtection/SmartM365-M365-BackupPolicyScope-Inventory.ps1`. |
...
| `DeviceManagementConfiguration.Read.All` | Reads Intune configurations, policies, and reports, including compliance policies, Windows Update for Business, Feature Update / Quality Update profiles, and report export jobs. | `SmartInventory/M365Inventory/IntuneInventory/Devices/SmartM365-Device-System-Inventory.ps1`; `SmartInventory/M365Inventory/IntuneInventory/Devices/SmartM365-Devices-Compliance-Inventory.ps1`; `SmartInventory/M365Inventory/IntuneInventory/WindowsUpdate/SmartM365-WinUpdate_Status_From_Intune.ps1`; `SmartInventory/M365Inventory/IntuneInventory/WindowsUpdate/AutopatchAlerts/SmartM365-Intune-WindowsAutopatch-Alerts-Inventory.ps1`. |
| `DeviceManagementManagedDevices.Read.All` | Reads Intune managed devices and their properties: device inventory, BIOS, compliance, system data, and upgrade eligibility. It can read existing export jobs but Microsoft does not document it as sufficient to create an export job. | `SmartInventory/M365Inventory/IntuneInventory/Devices/SmartM365-Devices-Inventory.ps1`; `SmartInventory/M365Inventory/IntuneInventory/Devices/SmartM365-Devices-BIOS-Inventory.ps1`; `SmartInventory/M365Inventory/IntuneInventory/Devices/SmartM365-Devices-Compliance-Inventory.ps1`; `SmartInventory/M365Inventory/IntuneInventory/Devices/SmartM365-Device-System-Inventory.ps1`; `SmartInventory/M365Inventory/IntuneInventory/Devices/SmartM365-Devices-UpgradeEligibility.ps1`; `SmartInventory/M365Inventory/IntuneInventory/Applications/SmartM365-Intune-DiscoveredApps-Inventory.ps1`; `SmartInventory/M365Inventory/IntuneInventory/WindowsUpdate/SmartM365-WinUpdate_Status_From_Intune.ps1`; `SmartInventory/M365Inventory/IntuneInventory/WindowsUpdate/AutopatchAlerts/SmartM365-Intune-WindowsAutopatch-Alerts-Inventory.ps1`. |
| `DeviceManagementScripts.Read.All` | Reads/exports Intune remediation scripts exposed as `deviceHealthScripts`, including details and assignments. | `SmartInventory/M365Inventory/IntuneInventory/SmartM365-Intune-ExportRemediationScripts.ps1`. |
...
| `Sites.Read.All` | Reads SharePoint Online sites, lists, drives, quota information, and team site storage metadata for inventory and monitoring. This is runtime read access and is separate from `Sites.Selected`, which is used for SmartM365 uploads only. | `SmartInventory/M365Inventory/SharePoint/SmartM365-SPO-Inventory.ps1`; `SmartInventory/M365Inventory/Teams/SmartM365-Teams-Inventory.ps1` for exact team SharePoint quota lookup. |
| `Team.ReadBasic.All` | Reads Teams basic metadata through `/teams/{id}` and related Teams endpoints. | `SmartInventory/M365Inventory/Teams/SmartM365-Teams-Inventory.ps1`. |
| `TeamMember.Read.All` | Reads Teams and group membership/ownership signals used to detect orphaned or under-owned teams. | `SmartInventory/M365Inventory/Teams/SmartM365-Teams-Inventory.ps1`. |
| `Channel.ReadBasic.All` | Reads Teams channel inventory. | `SmartInventory/M365Inventory/Teams/SmartM365-Teams-Inventory.ps1`. |
...
`SmartM365-TeamsPhonePstnUsage-Inventory.ps1 -IncludePhoneAssignments` has separate optional Teams PowerShell prerequisites. App-only `Connect-MicrosoftTeams` requires `Organization.Read.All` and a supported Teams Entra role on the service principal. The read-only collector uses only `Get-CsPhoneNumberAssignment`; `Teams Telephony Administrator` is the most voice-focused documented role. These optional prerequisites are not needed for the Graph call-usage CSV files.
...
| --- | --- | --- |
| `DeviceManagementApps.ReadWrite.All` | Broad permission requested by the current Autopatch script to access Intune/reporting data. Reduce it if the calls remain read-only. | `SmartInventory/M365Inventory/IntuneInventory/WindowsUpdate/AutopatchAlerts/SmartM365-Intune-WindowsAutopatch-Alerts-Inventory.ps1`. |
...
| `DeviceManagementManagedDevices.ReadWrite.All` | Required by Microsoft's create `deviceManagementExportJob` documentation. Endpoint Analytics and endpoint-security health use it only to POST temporary report export jobs and never send a device, configuration, policy, assignment, or remediation write. | `SmartInventory/M365Inventory/IntuneInventory/EndpointAnalytics/SmartM365-EndpointAnalytics-Inventory.ps1`; `SmartInventory/M365Inventory/IntuneInventory/Security/SmartM365-Intune-EndpointSecurityHealth-Inventory.ps1`; `SmartInventory/M365Inventory/IntuneInventory/WindowsUpdate/AutopatchAlerts/SmartM365-Intune-WindowsAutopatch-Alerts-Inventory.ps1`. |
...
| --- | --- | --- |
| `DeviceManagementScripts.ReadWrite.All` | Creates, updates, lists, duplicates, assigns, and deletes Intune remediation packages exposed by Microsoft Graph as `deviceHealthScripts`. | `Intune/Remediation/GUI/SmartM365-IntuneRemediation-GUI.ps1`. |
...
## PowerShell Module Baseline

| Module | Why it is needed | Install source |
| --- | --- | --- |
| `Microsoft.Graph.Authentication` | Graph authentication and `Invoke-MgGraphRequest` used by most Graph inventory scripts. | PSGallery |
...
| `MicrosoftPowerBIMgmt.Profile` | Official Power BI sign-in, certificate-based service principal authentication, and Power BI access-token retrieval for the Power BI and Fabric Activity Events collector. | PSGallery |
| `ActiveDirectory` | AD inventory and AD health scripts; installed through RSAT/Windows Server features, not PSGallery. | Windows feature |

`Setup/Install-SmartM365-SmartInventoryPrerequisites.ps1` installs PSGallery modules with `Install-Module -Scope CurrentUser -Force -AllowClobber` by default. Script preflights also auto-install missing PSGallery modules with the same flags, except for non-PSGallery modules such as `ActiveDirectory`.
...
`SmartInventory/M365Inventory/PowerBI/SmartM365-PowerBIFabricActivity-Inventory.ps1` calls `GET https://api.powerbi.com/v1.0/myorg/admin/activityevents`. Its token audience is the Power BI resource `https://analysis.windows.net/powerbi/api`, not Microsoft Graph.

| Authentication mode | Required access | Important constraint |
| --- | --- | --- |
| Interactive Fabric Administrator | Delegated `Tenant.Read.All` or `Tenant.ReadWrite.All` in the Power BI token. The signed-in user must hold the Fabric Administrator role. | This delegated scope is not added to the unattended SmartM365 application permission set. |
| Service principal certificate | In the Fabric admin portal, enable **Service principals can access read-only admin APIs** for a security group containing the SmartM365 service principal. The script reuses `AppId`, `TenantId`, and `Thumbprint` from the selected local tenant profile. | Do not assign admin-consent-required Power BI permissions to the Entra application. The Fabric tenant setting is the supported authorization mechanism for this read-only admin API. |

The read-only admin API tenant setting is distinct from the general developer setting that allows service principals to use Fabric APIs. `Setup/SmartM365-Create-AppRegistration.ps1` intentionally does not add arbitrary Power BI API permissions for this collector.
...
- Replace transitional Intune `ReadWrite` permissions with `Read` permissions where Microsoft documents a read-only alternative. Keep the Endpoint Analytics exception until Microsoft permits creating `deviceManagementExportJob` with a `Read.All` permission.
