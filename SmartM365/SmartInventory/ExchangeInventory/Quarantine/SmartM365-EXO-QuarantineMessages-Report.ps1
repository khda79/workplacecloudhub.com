...
try {
    ...
    if ($global:RetentionMaxCSV -gt 0 -and $MaxItems -eq 0) {
        ...
        Remove-SmartM365TimestampedFilesOlderThan -FolderPath $OutputPath -FilePattern 'Exchange_EXO_QuarantineMessages_*.csv' -RetentionDays 7 -ExcludeFiles $maxItemsCsvPaths -LogFile $global:LogTextFile
        Remove-SmartM365TimestampedFilesOlderThan -FolderPath $OutputPath -FilePattern 'Exchange_EXO_QuarantineMessages_*.xlsx' -RetentionDays 7 -ExcludeFiles $maxItemsWorkbookPaths -LogFile $global:LogTextFile
    }
    elseif ($MaxItems -gt 0) {
