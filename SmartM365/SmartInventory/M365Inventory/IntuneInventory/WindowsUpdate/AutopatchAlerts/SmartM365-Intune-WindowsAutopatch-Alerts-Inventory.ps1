...
function Invoke-GraphGetAll {
    ...
    do {
        if (-not $visited.Add($nextUri)) { throw 'Autopatch Graph pagination returned a repeated @odata.nextLink; collection is incomplete.' }
        $response = Invoke-AutopatchGraphRequest -Method GET -Uri $nextUri

        if ($null -eq $response) {
            throw 'Autopatch Graph pagination returned a null page; collection is incomplete.'
        }
        ...
        if (-not $hasValueProperty) {
            throw 'Autopatch Graph pagination returned an invalid collection response without a value property.'
        }
        if ($null -ne $value) {
...
try {
    ...
    Remove-CoreSmartM365TimestampedFilesOlderThan -FolderPath $OutputFolder -FilePattern '*.csv' -RetentionDays 7 -LogFile $LogFile
