...
set "CACHE_BASE="
call :TryCacheBase "%ProgramData%\SmartM365\LauncherCache\ExchangeLocalMailboxes\WithADPermissions"
if not defined CACHE_BASE call :TryCacheBase "%SystemRoot%\Temp\SmartM365LauncherCache\ExchangeLocalMailboxes\WithADPermissions"
if not defined CACHE_BASE call :TryCacheBase "C:\Temp\SmartM365LauncherCache\ExchangeLocalMailboxes\WithADPermissions"
if not defined CACHE_BASE call :TryCacheBase "%TEMP%\SmartM365LauncherCache\ExchangeLocalMailboxes\WithADPermissions"
if not defined CACHE_BASE (
    echo Failed to create a writable local cache base folder.
    echo Tried:
    echo   %ProgramData%\SmartM365\LauncherCache\ExchangeLocalMailboxes\WithADPermissions
    echo   %SystemRoot%\Temp\SmartM365LauncherCache\ExchangeLocalMailboxes\WithADPermissions
    echo   C:\Temp\SmartM365LauncherCache\ExchangeLocalMailboxes\WithADPermissions
    echo   %TEMP%\SmartM365LauncherCache\ExchangeLocalMailboxes\WithADPermissions
    pause
    exit /b 1
)
goto :CacheBaseReady
...
"%POWERSHELL5%" -NoProfile -ExecutionPolicy Bypass -File "%LOCAL_SCRIPT_DIR%\SmartM365-Exchange-Local-Mailboxes-Inventory.ps1" -Tenant "%SMARTM365_LAUNCHER_TENANT%" -IncludeADPermission
