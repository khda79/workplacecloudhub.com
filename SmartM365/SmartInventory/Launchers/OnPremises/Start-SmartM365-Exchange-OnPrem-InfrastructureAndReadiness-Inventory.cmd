...
pushd "%~dp0..\..\ExchangeInventory\OnPremises\ServersAndStorage\" >nul 2>&1
if errorlevel 1 (
    echo Failed to switch to the launcher directory.
    echo Launcher path:
    echo   %~dp0..\..\ExchangeInventory\OnPremises\ServersAndStorage\
    pause
    exit /b 1
)
...
set "LOCAL_SCRIPT_DIR=%LOCAL_SMARTM365_ROOT%\SmartInventory\ExchangeInventory\OnPremises\ServersAndStorage"
...
"%POWERSHELL5%" -NoProfile -ExecutionPolicy Bypass -File "%LOCAL_SCRIPT_DIR%\SmartM365-Exchange-OnPrem-InfrastructureAndReadiness-Inventory.ps1" -Tenant "%SMARTM365_LAUNCHER_TENANT%" %*
