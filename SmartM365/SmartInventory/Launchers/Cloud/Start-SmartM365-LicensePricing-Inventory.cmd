...
set "UNC_WORK_DIR=%~dp0..\..\M365Inventory\Licensing\."
set "SCRIPT_PATH=%UNC_WORK_DIR%\SmartM365-LicensePricing-Inventory.ps1"
set "PRICE_SOURCE=%UNC_WORK_DIR%\M365_License_Prices_Public-France.csv"
set "PWSH="
...
if not exist "%PRICE_SOURCE%" (
    call :Log "Pricing source not found: %PRICE_SOURCE%"
    exit /b 3
)

"%PWSH%" -NoProfile -ExecutionPolicy Bypass -File "%SCRIPT_PATH%" -Tenant prod -PricingSourceCsvPath "%PRICE_SOURCE%" -EnableConfiguredExternalActions %*
set "EXIT_CODE=%ERRORLEVEL%"
if not "%EXIT_CODE%"=="0" call :Log "License pricing inventory failed with exit code %EXIT_CODE%."
exit /b %EXIT_CODE%
