...
function Get-RunningJobTimeoutWindow {
    ...
    # A supervised process may outlive its timeout when termination fails.
    $supervisionSafeUntilUtc = $Now.ToUniversalTime().AddMinutes([math]::Max(1, [int]$script:Settings.ElectionClaimGraceMinutes))
    if ($supervisionSafeUntilUtc -gt $leaseSafeUntilUtc) { $leaseSafeUntilUtc = $supervisionSafeUntilUtc }
    return [pscustomobject]@{
        TimeoutMinutes = $timeoutMinutes
        StartUtc = $startUtc
...
function Sync-RunningJobConcurrencyLease {
    ...
    $window = Get-RunningJobTimeoutWindow -JobName $JobName -RunInfo $RunInfo -Now $Now
    try {
        $leaseParameters = @{
...
function Update-RunningJobs {
    ...
    foreach ($name in @($script:RunningJobs.Keys)) {
        ...
        try {
            ...
            Write-OrchestratorRuntimeUpdateWarning -Key ("process-probe:{0}" -f $name) -Message ("Job {0}: process exit could not be confirmed; supervision and concurrency protection are retained." -f $name) -Now $Now
        ...
        $timeoutWindow = Get-RunningJobTimeoutWindow -JobName $name -RunInfo $info -Now $Now
        $timeoutRequested = $info.ContainsKey('TimeoutRequested') -and $info.TimeoutRequested
        if ($timeoutRequested -or ($timeoutWindow.TimeoutMinutes -gt 0 -and $Now.ToUniversalTime() -gt $timeoutWindow.TimeoutUtc)) {
            if (-not $timeoutRequested) {
                $info['TimeoutRequested'] = $true
                $state = Get-JobState -JobName $name
                if ($null -ne $state.Running) { $state.Running['TimeoutRequested'] = $true }
                Save-OrchestratorState
            }
            Sync-RunningJobConcurrencyLease -JobName $name -RunInfo $info -Now $Now
            if (-not (Test-OrchestratorStatePersistenceReady)) {
                Write-OrchestratorRuntimeUpdateWarning -Key ("timeout-state:{0}" -f $name) -Message ("Job {0}: timeout termination is deferred until running state can be persisted; supervision is retained." -f $name) -Now $Now
                continue
            }
            ...
            try { $process.Refresh(); $processStillRunning = -not $process.HasExited } catch { }
            if ($processStillRunning) {
                Write-OrchestratorLog -Message ("Job {0}: exit of PID {1} is not confirmed after timeout kill request; retaining supervision and concurrency protection. Review the job log and launcher." -f $name, $process.Id) -Level WARN
                continue
            }
            else {
...
try {
    ...
    if ($script:Settings.CentralConfigurationEnabled) {
        ...
        $committedJobsTemplatePath = Join-Path -Path $PSScriptRoot -ChildPath 'Orchestrator-Jobs.json.template'
        $centralSnapshot = Initialize-SmartM365OrchestratorCentralConfiguration -SharedDataFolderPath $sharedDataFolder -BootstrapJobsPath $bootstrapJobsPath -BootstrapClusterDocument $bootstrapCluster -TemplateJobsPath $committedJobsTemplatePath
...
try {
    ...
    $manifestSync = if ($script:Settings.CentralConfigurationEnabled) {
    ...
    if ($null -ne $manifestSync -and $manifestSync.Created) {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCN+KzkQPKYEvH6
# Oa+qHATZSvnebclMoJIQybbGKh4qPaCCF/swggS9MIIDJaADAgECAhAebu87xzjh
...
# hvcNAQkEMSIE