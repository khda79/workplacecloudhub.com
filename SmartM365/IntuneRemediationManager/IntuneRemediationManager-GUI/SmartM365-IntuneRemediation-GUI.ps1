...
function Write-GuiLog {
    ...
    $fileLine = '[{0}] {1}' -f (Get-Date -Format 'yyyy-MM-dd HH:mm:ss'), $Message
...
function Export-LocalRemediationsArchive {
    ...
    try {
        ...
        Copy-Item -LiteralPath $localRoot -Destination (Join-Path -Path $localExportRoot -ChildPath 'IntuneRemediation') -Recurse -Force
