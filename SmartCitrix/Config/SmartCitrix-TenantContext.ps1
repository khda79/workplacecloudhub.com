...
function Get-SmartCitrixJsonTemplatePath {
    [CmdletBinding()]
...
function Add-SmartCitrixMissingJsonTemplateProperties {
    [CmdletBinding()]
    param(
    ...
    foreach ($property in $Template.PSObject.Properties) {
        ...
        if ($null -ne $currentValue -and $null -ne $property.Value -and $currentValue -is [pscustomobject] -and $property.Value -is [pscustomobject]) {
            foreach ($nestedPath in (Add-SmartCitrixMissingJsonTemplateProperties -Target $currentValue -Template $property.Value -PrefixPath $propertyPath)) {
...
function Sync-SmartCitrixJsonConfigWithTemplate {
    [CmdletBinding()]
    param(
    ...
    if ([string]::IsNullOrWhiteSpace($TemplatePath)) {
        $TemplatePath = Get-SmartCitrixJsonTemplatePath -Path $Path
    }
    ...
    try {
        ...
        $addedKeys = @(Add-SmartCitrixMissingJsonTemplateProperties -Target $Config -Template $template)
...
function Read-SmartCitrixJsonConfig {
    ...
    try {
        $config = ConvertTo-SmartCitrixHashtable -InputObject (Get-Content -LiteralPath $Path -Raw -ErrorAction Stop | ConvertFrom-Json -ErrorAction Stop)
        return (Sync-SmartCitrixJsonConfigWithTemplate -Config $config -Path $Path)
    }
    catch {
...
function Get-SmartCitrixScriptLocalConfig {
    ...
    try {
        ...
        return (Sync-SmartCitrixJsonConfigWithTemplate -Config $config -Path $configPath)
    }
    catch {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCTPTFrj5fgSfJV
# S7aBNrHyjCIzKyOz3vBbpRm6fL9KZaCCBBQwggQQMIICeKADAgECAhBwIfLVIgJW
...
# BDEiBCCDJHsB35JvoiSakXL9FZ9QTtBqkLe1E4T1E8PwT+SRiDANBgkqhkiG9w0B
# AQEFAASCAYA4pPV/5I5SQTgpcBWBX86vIbdezWDJg1UtDGvOz7IngKp6HWfnYaE2
# HdCb3d+4GNMDQkuwENmcOFQbHdAI04pzH53c6wcPFSvh6QnF5aZwrI97r1U80DFB
# fYGSPp8+crHT7gYdq0hfG9yMB87qLjMTTue6i9nFD/vosUM3CExD/WDS/MD5rbqy
# isnHJ017DFqBkTxEkGE0Y3Gayq0L5RhEocJDE3mOwmOH7QRogCcgAImcSpR8vd6+
# L2BfOJTj4kG6StrGfwNtrWArMue0EBJEee4roMpErhCTzBVWgXz+ZgOrO6ItqXcW
# OuaLiCPS0LrLEPqEkC87k32kWJDBtnttgkxoOC5N0QujGBqIkXOrzgXq9KokxRqd
# e1I4saBOmE5Yn7DrYpbjWCrxl6v3RLTJlALvG4Co9UC24xbQVT4PSTfdf3rZdpBk
# GNFdiw7RJisEx+T8+xJOlraafnpZAvhCh4wNbdV0+2EnKckLndhwFgRWBukx61t/
# urYDUTFxy+8=
